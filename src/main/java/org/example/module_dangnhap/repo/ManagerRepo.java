...
@Repository
public interface ManagerRepo extends JpaRepository<InforUser,Long> {
    @Query(value = "SELECT u.id, u.name, u.email, u.phone, u.address, u.account_id " +
            ...
            "WHERE r.role_name IN ( 'RECEPTIONIST') " +
            ...
            countQuery = "SELECT COUNT(*) FROM infor_user u " +
                    ...
                    "AND u.account_id != :currentAccountId",
            nativeQuery = true)
    Page<InforUser> findAlReceptionist(@Param("currentAccountId") Long currentAccountId, Pageable pageable);
}
